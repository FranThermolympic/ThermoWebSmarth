...
namespace ThermoWeb.MATERIALES
{
    public partial class UbicacionMaterialesLITE : System.Web.UI.Page
    {
        ...
        public void Rellenar_Grid_AUX(object sender, EventArgs e)
        {
            try
            {
        ...
        public static bool CapturarQRPistola(string MATERIAL, string UBICACION)
        {
            //int BIT = 0;
            string[] DATOSQR = MATERIAL.Split(new char[] { '_' });
            string producto = "";
            string cantidad = "";
            string lote = "";
            ...
            if (DATOSQR[0].ToString().Substring(0, 8) == "[]>E2*06")
                {
                    producto = DATOSQR[1].ToString().Remove(0, 1);
                }
            if (DATOSQR[2].ToString().Substring(0, 1) == "Q")
                {
                cantidad = DATOSQR[2].ToString().Remove(0, 1);
                }
            if (DATOSQR[3].ToString().Substring(0, 1) == "T")
                {
                     lote = DATOSQR[3].ToString().Remove(0, 1);
                }
            if (DATOSQR[4].ToString().Substring(0, 2) == "1T")
                {
                     loteproveedor = DATOSQR[4].ToString().Remove(0, 2);
                }
            if (DATOSQR[5].ToString().Substring(0, 1) == "V")
                {
                     proveedor = DATOSQR[5].ToString().Remove(0, 1);
                }
            if (DATOSQR[6].ToString().Substring(0, 1) == "K")
                {
                     pedidoventa = DATOSQR[6].ToString().Substring(0, DATOSQR[6].Length - 5).Remove(0, 1);
                }

               // string[] RecorteMAT = NUMMaterial.Value.Split(new char[] { '¬' });
                //string MATERIAL = RecorteMAT[0].ToString();
            Conexion_MATERIALES conexion = new Conexion_MATERIALES();
            if (producto != "" && UBICACION.Substring(0,2) != "..")
            { 
             conexion.Insertar_MaterialxUbicacionxLote(UBICACION, producto, "0",Convert.ToInt32(cantidad.Replace(".", "")),lote,loteproveedor,proveedor,pedidoventa);
            }
