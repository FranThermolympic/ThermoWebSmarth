<%@ Page Language="C#" AutoEventWireup="True" CodeBehind="UbicacionMateriasPrimasLITE.aspx.cs" Inherits="ThermoWeb.MATERIALES.UbicacionMaterialesLITE" MasterPageFile="~/SMARTHLITE.Master"
...
<asp:Content ID="Cuerpo" ContentPlaceHolderID="MainContent" runat="server">
    ...
    window.onload = function () {
        ...
        document.getElementById("PILL_ESTPROD").onclick = function CargaESTPROD() {
            document.getElementById("BTN_PR0_M1").onclick();
        }
        document.getElementById("NUMMaterialPistola").onchange = function CargaEnvíaDatosQR() {
            EnviarDatosLectoraQR();
    ...
    <script type="text/javascript">
        ...
        function InsertarQR(codigo) {
            //document.getElementById('NUMMaterial').hidden = false;
            //document.getElementById('NUMMaterialPistola').hidden = true;
            const first = codigo.slice(0, 1);
            const first3 = codigo.slice(0, 3);
            if (first3 == "30S") {
                document.getElementById("NUMMaterial").value = codigo.slice(3, 12);
                document.getElementById("BTNAgregarMaterial").click();
            }
        ...
        function LectoraCamaraOLD() {
            document.getElementById('NUMMaterialManual').hidden = false;
            document.getElementById('NUMMaterialPistola').hidden = true;
            fully.scanQrCode('Prompt text', 'javascript:InsertarQR(\'$code\');');

        }
        function LectoraCamara() {
            document.getElementById('NUMMaterialManual').hidden = true;
            document.getElementById('NUMMaterialPistola').hidden = false;
            document.getElementById('NUMMaterialPistola').value = "";
            document.getElementById('NUMMaterialPistola').focus();
            document.getElementById('NUMMaterialPistola').select();
            fully.scanQrCode('Prompt text', 'javascript:EnviarDatosCamaraQR(\'$code\');');
        ...
        function EnviarDatosCamaraQR(codigo) {
            //document.getElementById('NUMMaterialPistola').value = codigo;
            var codigoDeBarras = codigo;
            if (codigoDeBarras.startsWith("[]>E2*06") && codigoDeBarras.endsWith("E2*11")) {
                $.ajax({
                    type: "POST",
                    url: "UbicacionMateriasPrimasLITE.aspx/CapturarQRPistola",
                    data: "{MATERIAL: '" + codigoDeBarras + "', UBICACION: '" + document.getElementById('AuxTBUbicacion').innerText + "'}",
                    contentType: "application/json; charset=utf-8",
                    //dataType: "json",
                    success: function () {
                        document.getElementById("btnRellenarGridAUX").click();
                        //alert("Vamos");
                ...
                alert("El código leído no es válido");
                document.getElementById("btnPopDocVinculados").click();
            }
            //alert("salgo");

        }
        function EnviarDatosLectoraQR() {
            var codigoDeBarras = document.getElementById('NUMMaterialPistola').value;
            if (codigoDeBarras.startsWith("[]>E2*06") && codigoDeBarras.endsWith("E2*11")) {
                $.ajax({
                    type: "POST",
                    url: "UbicacionMateriasPrimasLITE.aspx/CapturarQRPistola",
        ...
        function AlertaFaltanDatos() {
            alert("Debes rellenar el número de material y el lote para poder realizar la operación.");
            ShowPopDocVinculados();
        }
    </script>
    <style>
        .ps-25 {
    ...
    <div style="background: url(LOGOFONDOTHERMO.png) right top no-repeat">
        ...
        <button type="button" id="btnRellenarGridAUX" class="btn btn-primary invisible" runat="server" onserverclick="Rellenar_Grid_AUX">boton</button>
                ...
                <div class="row">
                    <div class="col-lg-2 p-0 ">
                        ...
                        <ul class="nav flex-column nav-pills rounded rounded-2 border border-secondary shadow bg-white mt-0" style="width: 100%" id="pills-tab" role="tablist">
                            <h6 class="ms-2 mt-1" style="font-style: italic"><i class="bi bi-layout-sidebar-inset"></i>&nbsp EN PRODUCCIÓN</h6>
                            <li class="nav-item ms-1 me-1" role="presentation" id="Li7" runat="server">
                                <button class="nav-link shadow  border border-secondary" runat="server" id="PILL_ESTPROD" data-bs-toggle="pill" data-bs-target="#TAB_EST_PROD" type="button" role="tab" aria-controls="TAB_EST_PROD" aria-selected="true" style="font-weight: bold; width: 100%"><i class="bi bi-gear-wide-connected"></i>&nbsp EN PRODUCCION</button>
                            </li>
                            <h6 class="ms-2 mt-3 " style="font-style: italic"><i class="bi bi-layout-sidebar-inset"></i>&nbsp SILOS</h6>
                            ...
                            <div class="container-fluid shadow">
                                ...
                                <div class="row">
                                    <div class="tab-content p-0">
                                        <div id="TAB_EST_PROD" class="tab-pane fade" runat="server">
                                            <div class="col-lg-12 p-0" style="background-color: gainsboro">
                                                <button type="button" id="BTN_PR0_M1" runat="server" class="btn btn-outline-dark" style="width: 100%; font-weight: bold" onserverclick="Rellenar_Grid"><i class="bi bi-grid-3x3"></i>&nbsp EN PRODUCCION</button>
                                            </div>
                                        </div>
                                ...
                                <div class="row" runat="server" id="AltEnproduccion">
                                    <div class="col-lg-12 p-0 border border-dark">
                                        <asp:GridView ID="GridEnProduccion" runat="server" AllowSorting="True" Style="width: 100%" CssClass="table border border-dark border-start-0 border-end-0 mb-0" AutoGenerateColumns="false" ShowHeader="true" OnRowCommand="GridCommandEventHandler">
                                            ...
                                            <Columns>
                                                <asp:TemplateField ItemStyle-Width="10%" ItemStyle-HorizontalAlign="Left" ItemStyle-Font-Size="Large" ItemStyle-BackColor="CornflowerBlue" ItemStyle-ForeColor="White">
                                                    ...
                                                    <ItemTemplate>
                                                        <asp:Label ID="lblSilo" Font-Italic="true" Font-Bold="true" runat="server" Text='<%#Eval("Ubicacion")  %>' />
                                                    </ItemTemplate>
                                                </asp:TemplateField>
                                                <asp:TemplateField ItemStyle-Width="90%" ItemStyle-HorizontalAlign="Left" ItemStyle-Font-Size="Large">
                                                    <ItemTemplate>
                                                        <asp:LinkButton type="button" runat="server" ID="BTNEliminaArticulo" OnClientClick="return confirm('Vas a eliminar este artículo de la ubicación, ¿estás seguro?');" CommandName="EliminaArticulo" CommandArgument='<%#Eval("Fechaentrada","{0:dd/MM/yyyy}") + "¬" + Eval("Articulo") + "¬" + Eval("Ubicacion")+ "¬" + Eval("Lote") %>'  UseSubmitBehavior="true" CssClass="btn  btn-danger border border-dark shadow" Style="font-size: 1rem">
                                                            <i class="bi bi-x"></i>
                                                        </asp:LinkButton>
                                                        <asp:Label ID="lblArticuloText" Font-Italic="true" Font-Bold="true" runat="server" Text='<%#Eval("Articulo") + " " + Eval("Descripcion")  %>' />
                                                        <asp:Label ID="lblLote" Font-Italic="true" Font-Bold="true" runat="server" Text='<%#"| Lote: " + Eval("Lote") %>' />
                                                        ...
                                                        <asp:HiddenField ID="lblEnProduccion" runat="server" Value='<%#Eval("EnMaquina")%>' />
                                                        <asp:Image runat="server" ID="IMGEnMaquina" Width="45px" Visible="false" ImageUrl="..\\SMARTH_fonts\\INTERNOS\\iconenprod.png" />
                                                        <asp:LinkButton type="button" runat="server" ID="BTNBloquearProduccion" UseSubmitBehavior="true" CssClass="btn  btn-primary border border-dark float-end shadow" CommandName="BloqueaArticulo" CommandArgument='<%#Eval("Fechaentrada","{0:dd/MM/yyyy}") + "¬" + Eval("Articulo") + "¬" + Eval("Ubicacion") + "¬" + Eval("EnMaquina") + "¬" + Eval("Lote") %>' Style="font-size: 1rem">
                                                                        <i class="bi bi-grid-3x3">&nbsp Devolver a almacén</i>
                                                        </asp:LinkButton>
                                                    </ItemTemplate>
                                                </asp:TemplateField>
                                                ...
                                                <asp:TemplateField ItemStyle-Width="90%" ItemStyle-HorizontalAlign="Left" ItemStyle-Font-Size="Large">
                                                    <ItemTemplate>
                                                        <div class="btn-group me-2">
                                                            <asp:LinkButton type="button" runat="server" ID="BTNAuditaUbicacion" CommandName="ValidaArticulo" CommandArgument='<%#Eval("Articulo") + "¬" + Eval("NombreUbicacion") %>' UseSubmitBehavior="true" CssClass="btn  btn-success border border-dark shadow" Style="font-size: 1rem">
                                                            <i class="bi bi-check-lg"></i>
                                                            </asp:LinkButton>
                                                            <asp:LinkButton type="button" runat="server" ID="BTNEliminaArticulo" OnClientClick="return confirm('Vas a eliminar este artículo de la ubicación, ¿estás seguro?');" CommandName="EliminaArticulo" CommandArgument='<%#Eval("Fechaentrada","{0:dd/MM/yyyy}") + "¬" + Eval("Articulo") + "¬" + Eval("NombreUbicacion") %>' UseSubmitBehavior="true" CssClass="btn  btn-danger border border-dark shadow" Style="font-size: 1rem">
                                                            <i class="bi bi-x"></i>
                                                            </asp:LinkButton>
                                                            <asp:LinkButton type="button" runat="server" ID="BTNBloquearProduccion" Visible="false" UseSubmitBehavior="true" CssClass="btn  btn-primary border border-dark shadow" Style="font-size: 1rem">
                                                        ...
                                                        <asp:Label ID="lblFechaEntrada" Font-Size="Smaller" Visible="false" runat="server" Font-Italic="true" Text='<%#"<br />(Desde el " + Eval("Fechaentrada","{0:dd/MM/yyyy}") + ")" %>' />
                                                        <asp:Label ID="lblAuxFechaEntrada" Visible="false" Font-Size="Smaller" runat="server" Font-Italic="true" Text='<%#Eval("Fechaentrada","{0:dd/MM/yyyy}") %>' />
                                                        <asp:LinkButton type="button" runat="server" ID="BTNAgregaLinea" CommandName="AgregaArticuloSILO" CommandArgument='<%#Eval("NombreUbicacion")  %>' UseSubmitBehavior="true" CssClass="btn btn-light border border-dark shadow float-end" Style="font-size: 1rem">
                                                ...
                                                <asp:GridView ID="GridE1" runat="server" AllowSorting="True" Style="width: 100%" CssClass="table border border-dark border-start-0 border-end-0 mb-0" AutoGenerateColumns="false" ShowHeader="true" OnRowDataBound="GridView_RowDataBound" OnRowCommand="GridCommandEventHandler">
                                                    ...
                                                    <Columns>
                                                        <asp:TemplateField ItemStyle-Width="90%" ItemStyle-HorizontalAlign="Left" ItemStyle-Font-Size="Large">
                                                            ...
                                                            <ItemTemplate>
                                                                <div class="btn-group me-2">
                                                                    <asp:LinkButton type="button" runat="server" ID="BTNAuditaUbicacion" CommandName="ValidaArticulo" CommandArgument='<%#Eval("Articulo") + "¬" + Eval("Ubicacion") %>' UseSubmitBehavior="true" CssClass="btn  btn-success border border-dark shadow" Style="font-size: 1rem">
                                                            <i class="bi bi-check-lg"></i>
                                                                    </asp:LinkButton>
                                                                    <asp:LinkButton type="button" runat="server" ID="BTNEliminaArticulo" OnClientClick="return confirm('Vas a eliminar este artículo de la ubicación, ¿estás seguro?');" CommandName="EliminaArticulo" CommandArgument='<%#Eval("Fechaentrada","{0:dd/MM/yyyy}") + "¬" + Eval("Articulo") + "¬" + Eval("Ubicacion")+ "¬" + Eval("Lote") %>'  UseSubmitBehavior="true" CssClass="btn  btn-danger border border-dark shadow" Style="font-size: 1rem">
                                                            <i class="bi bi-x"></i>
                                                                    </asp:LinkButton>
                                                                    <asp:LinkButton type="button" runat="server" ID="BTNBloquearProduccion" CommandName="BloqueaArticulo" CommandArgument='<%#Eval("Fechaentrada","{0:dd/MM/yyyy}") + "¬" + Eval("Articulo") + "¬" + Eval("Ubicacion") + "¬" + Eval("EnMaquina") + "¬" + Eval("Lote") %>' UseSubmitBehavior="true" CssClass="btn  btn-primary border border-dark shadow" Style="font-size: 1rem">
                                                                        <i class="bi bi-gear-wide-connected"></i>
                                                                    </asp:LinkButton>
                                                                </div>
                                                                <asp:Label ID="lblArticuloText" Font-Italic="true" Font-Bold="true" runat="server" Text='<%#Eval("Articulo") + " " + Eval("Descripcion")  %>' />
                                                        ...
                                                        <asp:TemplateField ItemStyle-HorizontalAlign="Center" ItemStyle-VerticalAlign="Middle" HeaderStyle-Width="10%">
                                                            <HeaderTemplate>
                                                                <asp:LinkButton type="button" runat="server" ID="BTNAgregaLinea" CommandName="AgregaArticulo" CommandArgument="E1" UseSubmitBehavior="true" CssClass="btn btn-light border border-dark shadow float-end" Style="font-size: 1rem">
                                                            <i class="bi bi-box-arrow-in-right"></i>
                                                                </asp:LinkButton>
                                                            </HeaderTemplate>
                                                            <ItemTemplate>
                                            ...
                                            <div class="col-lg-3 border border-dark p-0" runat="server" id="COL_E2">
                                                ...
                                                <asp:GridView ID="GridE2" runat="server" AllowSorting="True" Style="width: 100%" CssClass="table border border-dark border-start-0 border-end-0 mb-0" AutoGenerateColumns="false" ShowHeader="true" OnRowDataBound="GridView_RowDataBound" OnRowCommand="GridCommandEventHandler">
                                                    ...
                                                    <Columns>
                                                        ...
                                                        <asp:TemplateField ItemStyle-HorizontalAlign="Center" ItemStyle-VerticalAlign="Middle" HeaderStyle-Width="10%">
                                                            <HeaderTemplate>
                                                                <asp:LinkButton type="button" runat="server" ID="BTNAgregaLinea" CommandName="AgregaArticulo" CommandArgument="E2" UseSubmitBehavior="true" CssClass="btn btn-light border border-dark shadow float-end" Style="font-size: 1rem">
                                                            <i class="bi bi-box-arrow-in-right"></i>
                                                                </asp:LinkButton>
                                                            </HeaderTemplate>
                                                            <ItemTemplate>
                                            ...
                                            <div class="col-lg-3 border border-dark p-0" runat="server" id="COL_E3">
                                                ...
                                                <asp:GridView ID="GridE3" runat="server" AllowSorting="True" Style="width: 100%" CssClass="table border border-dark border-start-0 border-end-0 mb-0" AutoGenerateColumns="false" ShowHeader="true" OnRowDataBound="GridView_RowDataBound" OnRowCommand="GridCommandEventHandler">
                                                    ...
                                                    <Columns>
                                                        ...
                                                        <asp:TemplateField ItemStyle-HorizontalAlign="Center" ItemStyle-VerticalAlign="Middle" HeaderStyle-Width="10%">
                                                            <HeaderTemplate>
                                                                <asp:LinkButton type="button" runat="server" ID="BTNAgregaLinea" CommandName="AgregaArticulo" CommandArgument="E3" UseSubmitBehavior="true" CssClass="btn btn-light border border-dark shadow float-end" Style="font-size: 1rem">
                                                            <i class="bi bi-box-arrow-in-right"></i>
                                                                </asp:LinkButton>
                                                            </HeaderTemplate>
                                                            <ItemTemplate>
                                            ...
                                            <div class="col-lg-3 border border-dark p-0" runat="server" id="COL_E4">
                                                ...
                                                <asp:GridView ID="GridE4" runat="server" AllowSorting="True" Style="width: 100%" CssClass="table border border-dark border-start-0 border-end-0 mb-0" AutoGenerateColumns="false" ShowHeader="true" OnRowDataBound="GridView_RowDataBound" OnRowCommand="GridCommandEventHandler">
                                                    ...
                                                    <Columns>
                                                        ...
                                                        <asp:TemplateField ItemStyle-HorizontalAlign="Center" ItemStyle-VerticalAlign="Middle" HeaderStyle-Width="10%">
                                                            <HeaderTemplate>
                                                                <asp:LinkButton type="button" runat="server" ID="BTNAgregaLinea" CommandName="AgregaArticulo" CommandArgument="E4" UseSubmitBehavior="true" CssClass="btn btn-light border border-dark shadow float-end" Style="font-size: 1rem">
                                                            <i class="bi bi-box-arrow-in-right"></i>
                                                                </asp:LinkButton>
                                                            </HeaderTemplate>
                                                            <ItemTemplate>
                                            ...
                                            <div class="col-lg-3 border border-dark p-0" runat="server" id="COL_D1">
                                                ...
                                                <asp:GridView ID="GridD1" runat="server" AllowSorting="True" Style="width: 100%" CssClass="table border border-dark border-start-0 border-end-0 mb-0" AutoGenerateColumns="false" ShowHeader="true" OnRowDataBound="GridView_RowDataBound" OnRowCommand="GridCommandEventHandler">
                                                    ...
                                                    <Columns>
                                                        ...
                                                        <asp:TemplateField ItemStyle-HorizontalAlign="Center" ItemStyle-VerticalAlign="Middle" HeaderStyle-Width="10%">
                                                            <HeaderTemplate>
                                                                <asp:LinkButton type="button" runat="server" ID="BTNAgregaLinea" CommandName="AgregaArticulo" CommandArgument="D1" UseSubmitBehavior="true" CssClass="btn btn-light border border-dark shadow float-end" Style="font-size: 1rem">
                                                            <i class="bi bi-box-arrow-in-right"></i>
                                                                </asp:LinkButton>
                                                            </HeaderTemplate>
                                                            <ItemTemplate>
                                            ...
                                            <div class="col-lg-3 border border-dark p-0" runat="server" id="COL_D2">
                                                ...
                                                <asp:GridView ID="GridD2" runat="server" AllowSorting="True" Style="width: 100%" CssClass="table border border-dark border-start-0 border-end-0 mb-0" AutoGenerateColumns="false" ShowHeader="true" OnRowDataBound="GridView_RowDataBound" OnRowCommand="GridCommandEventHandler">
                                                    ...
                                                    <Columns>
                                                        ...
                                                        <asp:TemplateField ItemStyle-HorizontalAlign="Center" ItemStyle-VerticalAlign="Middle" HeaderStyle-Width="10%">
                                                            <HeaderTemplate>
                                                                <asp:LinkButton type="button" runat="server" ID="BTNAgregaLinea" CommandName="AgregaArticulo" CommandArgument="D2" UseSubmitBehavior="true" CssClass="btn btn-light border border-dark shadow float-end" Style="font-size: 1rem">
                                                            <i class="bi bi-box-arrow-in-right"></i>
                                                                </asp:LinkButton>
                                                            </HeaderTemplate>
                                                            <ItemTemplate>
                                            ...
                                            <div class="col-lg-3 border border-dark p-0" runat="server" id="COL_D3">
                                                ...
                                                <asp:GridView ID="GridD3" runat="server" AllowSorting="True" Style="width: 100%" CssClass="table border border-dark border-start-0 border-end-0 mb-0" AutoGenerateColumns="false" ShowHeader="true" OnRowDataBound="GridView_RowDataBound" OnRowCommand="GridCommandEventHandler">
                                                    ...
                                                    <Columns>
                                                        ...
                                                        <asp:TemplateField ItemStyle-HorizontalAlign="Center" ItemStyle-VerticalAlign="Middle" HeaderStyle-Width="10%">
                                                            <HeaderTemplate>
                                                                <asp:LinkButton type="button" runat="server" ID="BTNAgregaLinea" CommandName="AgregaArticulo" CommandArgument="D3" UseSubmitBehavior="true" CssClass="btn btn-light border border-dark shadow float-end" Style="font-size: 1rem">
                                                            <i class="bi bi-box-arrow-in-right"></i>
                                                                </asp:LinkButton>
                                                            </HeaderTemplate>
                                                            <ItemTemplate>
                                            ...
                                            <div class="col-lg-3 border border-dark p-0" runat="server" id="COL_D4">
                                                ...
                                                <asp:GridView ID="GridD4" runat="server" AllowSorting="True" Style="width: 100%" CssClass="table border border-dark border-start-0 border-end-0 mb-0" AutoGenerateColumns="false" ShowHeader="true" OnRowDataBound="GridView_RowDataBound" OnRowCommand="GridCommandEventHandler">
                                                    ...
                                                    <Columns>
                                                        ...
                                                        <asp:TemplateField ItemStyle-HorizontalAlign="Center" ItemStyle-VerticalAlign="Middle" HeaderStyle-Width="10%">
                                                            <HeaderTemplate>
                                                                <asp:LinkButton type="button" runat="server" ID="BTNAgregaLinea" CommandName="AgregaArticulo" CommandArgument="D4" UseSubmitBehavior="true" CssClass="btn btn-light border border-dark shadow float-end" Style="font-size: 1rem">
                                                            <i class="bi bi-box-arrow-in-right"></i>
                                                                </asp:LinkButton>
                                                            </HeaderTemplate>
                                                            <ItemTemplate>
                                            ...
                                            <div class="col-lg-3 border border-dark p-0" runat="server" id="COL_C1">
                                                ...
                                                <asp:GridView ID="GridC1" runat="server" AllowSorting="True" Style="width: 100%" CssClass="table border border-dark border-start-0 border-end-0 mb-0" AutoGenerateColumns="false" ShowHeader="true" OnRowDataBound="GridView_RowDataBound" OnRowCommand="GridCommandEventHandler">
                                                    ...
                                                    <Columns>
                                                        ...
                                                        <asp:TemplateField ItemStyle-HorizontalAlign="Center" ItemStyle-VerticalAlign="Middle" HeaderStyle-Width="10%">
                                                            <HeaderTemplate>
                                                                <asp:LinkButton type="button" runat="server" ID="BTNAgregaLinea" CommandName="AgregaArticulo" CommandArgument="C1" UseSubmitBehavior="true" CssClass="btn btn-light border border-dark shadow float-end" Style="font-size: 1rem">
                                                            <i class="bi bi-box-arrow-in-right"></i>
                                                                </asp:LinkButton>
                                                            </HeaderTemplate>
                                                            <ItemTemplate>
                                            ...
                                            <div class="col-lg-3 border border-dark p-0" runat="server" id="COL_C2">
                                                ...
                                                <asp:GridView ID="GridC2" runat="server" AllowSorting="True" Style="width: 100%" CssClass="table border border-dark border-start-0 border-end-0 mb-0" AutoGenerateColumns="false" ShowHeader="true" OnRowDataBound="GridView_RowDataBound" OnRowCommand="GridCommandEventHandler">
                                                    ...
                                                    <Columns>
                                                        ...
                                                        <asp:TemplateField ItemStyle-HorizontalAlign="Center" ItemStyle-VerticalAlign="Middle" HeaderStyle-Width="10%">
                                                            <HeaderTemplate>
                                                                <asp:LinkButton type="button" runat="server" ID="BTNAgregaLinea" CommandName="AgregaArticulo" CommandArgument="C2" UseSubmitBehavior="true" CssClass="btn btn-light border border-dark shadow float-end" Style="font-size: 1rem">
                                                            <i class="bi bi-box-arrow-in-right"></i>
                                                                </asp:LinkButton>
                                                            </HeaderTemplate>
                                                            <ItemTemplate>
                                            ...
                                            <div class="col-lg-3 border border-dark p-0" runat="server" id="COL_C3">
                                                ...
                                                <asp:GridView ID="GridC3" runat="server" AllowSorting="True" Style="width: 100%" CssClass="table border border-dark border-start-0 border-end-0 mb-0" AutoGenerateColumns="false" ShowHeader="true" OnRowDataBound="GridView_RowDataBound" OnRowCommand="GridCommandEventHandler">
                                                    ...
                                                    <Columns>
                                                        ...
                                                        <asp:TemplateField ItemStyle-HorizontalAlign="Center" ItemStyle-VerticalAlign="Middle" HeaderStyle-Width="10%">
                                                            <HeaderTemplate>
                                                                <asp:LinkButton type="button" runat="server" ID="BTNAgregaLinea" CommandName="AgregaArticulo" CommandArgument="C3" UseSubmitBehavior="true" CssClass="btn btn-light border border-dark shadow float-end" Style="font-size: 1rem">
                                                            <i class="bi bi-box-arrow-in-right"></i>
                                                                </asp:LinkButton>
                                                            </HeaderTemplate>
                                                            <ItemTemplate>
                                            ...
                                            <div class="col-lg-3 border border-dark p-0" runat="server" id="COL_C4">
                                                ...
                                                <asp:GridView ID="GridC4" runat="server" AllowSorting="True" Style="width: 100%" CssClass="table border border-dark border-start-0 border-end-0 mb-0" AutoGenerateColumns="false" ShowHeader="true" OnRowDataBound="GridView_RowDataBound" OnRowCommand="GridCommandEventHandler">
                                                    ...
                                                    <Columns>
                                                        ...
                                                        <asp:TemplateField ItemStyle-HorizontalAlign="Center" ItemStyle-VerticalAlign="Middle" HeaderStyle-Width="10%">
                                                            <HeaderTemplate>
                                                                <asp:LinkButton type="button" runat="server" ID="BTNAgregaLinea" CommandName="AgregaArticulo" CommandArgument="C4" UseSubmitBehavior="true" CssClass="btn btn-light border border-dark shadow float-end" Style="font-size: 1rem">
                                                            <i class="bi bi-box-arrow-in-right"></i>
                                                                </asp:LinkButton>
                                                            </HeaderTemplate>
                                                            <ItemTemplate>
                                            ...
                                            <div class="col-lg-3 border border-dark p-0" runat="server" id="COL_B1">
                                                ...
                                                <asp:GridView ID="GridB1" runat="server" AllowSorting="True" Style="width: 100%" CssClass="table border border-dark border-start-0 border-end-0 mb-0" AutoGenerateColumns="false" ShowHeader="true" OnRowDataBound="GridView_RowDataBound" OnRowCommand="GridCommandEventHandler">
                                                    ...
                                                    <Columns>
                                                        ...
                                                        <asp:TemplateField ItemStyle-HorizontalAlign="Center" ItemStyle-VerticalAlign="Middle" HeaderStyle-Width="10%">
                                                            <HeaderTemplate>
                                                                <asp:LinkButton type="button" runat="server" ID="BTNAgregaLinea" CommandName="AgregaArticulo" CommandArgument="B1" UseSubmitBehavior="true" CssClass="btn btn-light border border-dark shadow float-end" Style="font-size: 1rem">
                                                            <i class="bi bi-box-arrow-in-right"></i>
                                                                </asp:LinkButton>
                                                            </HeaderTemplate>
                                                            <ItemTemplate>
                                            ...
                                            <div class="col-lg-3 border border-dark p-0" runat="server" id="COL_B2">
                                                ...
                                                <asp:GridView ID="GridB2" runat="server" AllowSorting="True" Style="width: 100%" CssClass="table border border-dark border-start-0 border-end-0 mb-0" AutoGenerateColumns="false" ShowHeader="true" OnRowDataBound="GridView_RowDataBound" OnRowCommand="GridCommandEventHandler">
                                                    ...
                                                    <Columns>
                                                        ...
                                                        <asp:TemplateField ItemStyle-HorizontalAlign="Center" ItemStyle-VerticalAlign="Middle" HeaderStyle-Width="10%">
                                                            <HeaderTemplate>
                                                                <asp:LinkButton type="button" runat="server" ID="BTNAgregaLinea" CommandName="AgregaArticulo" CommandArgument="B2" UseSubmitBehavior="true" CssClass="btn btn-light border border-dark shadow float-end" Style="font-size: 1rem">
                                                            <i class="bi bi-box-arrow-in-right"></i>
                                                                </asp:LinkButton>
                                                            </HeaderTemplate>
                                                            <ItemTemplate>
                                            ...
                                            <div class="col-lg-3 border border-dark p-0" runat="server" id="COL_B3">
                                                ...
                                                <asp:GridView ID="GridB3" runat="server" AllowSorting="True" Style="width: 100%" CssClass="table border border-dark border-start-0 border-end-0 mb-0" AutoGenerateColumns="false" ShowHeader="true" OnRowDataBound="GridView_RowDataBound" OnRowCommand="GridCommandEventHandler">
                                                    ...
                                                    <Columns>
                                                        ...
                                                        <asp:TemplateField ItemStyle-HorizontalAlign="Center" ItemStyle-VerticalAlign="Middle" HeaderStyle-Width="10%">
                                                            <HeaderTemplate>
                                                                <asp:LinkButton type="button" runat="server" ID="BTNAgregaLinea" CommandName="AgregaArticulo" CommandArgument="B3" UseSubmitBehavior="true" CssClass="btn btn-light border border-dark shadow float-end" Style="font-size: 1rem">
                                                            <i class="bi bi-box-arrow-in-right"></i>
                                                                </asp:LinkButton>
                                                            </HeaderTemplate>
                                                            <ItemTemplate>
                                            ...
                                            <div class="col-lg-3 border border-dark p-0" runat="server" id="COL_B4">
                                                ...
                                                <asp:GridView ID="GridB4" runat="server" AllowSorting="True" Style="width: 100%" CssClass="table border border-dark border-start-0 border-end-0 mb-0" AutoGenerateColumns="false" ShowHeader="true" OnRowDataBound="GridView_RowDataBound" OnRowCommand="GridCommandEventHandler">
                                                    ...
                                                    <Columns>
                                                        ...
                                                        <asp:TemplateField ItemStyle-HorizontalAlign="Center" ItemStyle-VerticalAlign="Middle" HeaderStyle-Width="10%">
                                                            <HeaderTemplate>
                                                                <asp:LinkButton type="button" runat="server" ID="BTNAgregaLinea" CommandName="AgregaArticulo" CommandArgument="B4" UseSubmitBehavior="true" CssClass="btn btn-light border border-dark shadow float-end" Style="font-size: 1rem">
                                                            <i class="bi bi-box-arrow-in-right"></i>
                                                                </asp:LinkButton>
                                                            </HeaderTemplate>
                                                            <ItemTemplate>
                                            ...
                                            <div class="col-lg-3 border border-dark p-0" runat="server" id="COL_A1">
                                                ...
                                                <asp:GridView ID="GridA1" runat="server" AllowSorting="True" Style="width: 100%" CssClass="table border border-dark border-start-0 border-end-0 mb-0" AutoGenerateColumns="false" ShowHeader="true" OnRowDataBound="GridView_RowDataBound" OnRowCommand="GridCommandEventHandler">
                                                    ...
                                                    <Columns>
                                                        ...
                                                        <asp:TemplateField ItemStyle-HorizontalAlign="Center" ItemStyle-VerticalAlign="Middle" HeaderStyle-Width="10%">
                                                            <HeaderTemplate>
                                                                <asp:LinkButton type="button" runat="server" ID="BTNAgregaLinea" CommandName="AgregaArticulo" CommandArgument="A1" UseSubmitBehavior="true" CssClass="btn btn-light border border-dark shadow float-end" Style="font-size: 1rem">
                                                            <i class="bi bi-box-arrow-in-right"></i>
                                                                </asp:LinkButton>
                                                            </HeaderTemplate>
                                                            <ItemTemplate>
                                            ...
                                            <div class="col-lg-3 border border-dark p-0" runat="server" id="COL_A2">
                                                ...
                                                <asp:GridView ID="GridA2" runat="server" AllowSorting="True" Style="width: 100%" CssClass="table border border-dark border-start-0 border-end-0 mb-0" AutoGenerateColumns="false" ShowHeader="true" OnRowDataBound="GridView_RowDataBound" OnRowCommand="GridCommandEventHandler">
                                                    ...
                                                    <Columns>
                                                        ...
                                                        <asp:TemplateField ItemStyle-HorizontalAlign="Center" ItemStyle-VerticalAlign="Middle" HeaderStyle-Width="10%">
                                                            <HeaderTemplate>
                                                                <asp:LinkButton type="button" runat="server" ID="BTNAgregaLinea" CommandName="AgregaArticulo" CommandArgument="A2" UseSubmitBehavior="true" CssClass="btn btn-light border border-dark shadow float-end" Style="font-size: 1rem">
                                                            <i class="bi bi-box-arrow-in-right"></i>
                                                                </asp:LinkButton>
                                                            </HeaderTemplate>
                                                            <ItemTemplate>
                                            ...
                                            <div class="col-lg-3 border border-dark p-0" runat="server" id="COL_A3">
                                                ...
                                                <asp:GridView ID="GridA3" runat="server" AllowSorting="True" Style="width: 100%" CssClass="table border border-dark border-start-0 border-end-0 mb-0" AutoGenerateColumns="false" ShowHeader="true" OnRowDataBound="GridView_RowDataBound" OnRowCommand="GridCommandEventHandler">
                                                    ...
                                                    <Columns>
                                                        ...
                                                        <asp:TemplateField ItemStyle-HorizontalAlign="Center" ItemStyle-VerticalAlign="Middle" HeaderStyle-Width="10%">
                                                            <HeaderTemplate>
                                                                <asp:LinkButton type="button" runat="server" ID="BTNAgregaLinea" CommandName="AgregaArticulo" CommandArgument="A3" UseSubmitBehavior="true" CssClass="btn btn-light border border-dark shadow float-end" Style="font-size: 1rem">
                                                            <i class="bi bi-box-arrow-in-right"></i>
                                                                </asp:LinkButton>
                                                            </HeaderTemplate>
                                                            <ItemTemplate>
                                            ...
                                            <div class="col-lg-3 border border-dark p-0" runat="server" id="COL_A4">
                                                ...
                                                <asp:GridView ID="GridA4" runat="server" AllowSorting="True" Style="width: 100%" CssClass="table border border-dark border-start-0 border-end-0 mb-0" AutoGenerateColumns="false" ShowHeader="true" OnRowDataBound="GridView_RowDataBound" OnRowCommand="GridCommandEventHandler">
                                                    ...
                                                    <Columns>
                                                        ...
                                                        <asp:TemplateField ItemStyle-HorizontalAlign="Center" ItemStyle-VerticalAlign="Middle" HeaderStyle-Width="10%">
                                                            <HeaderTemplate>
                                                                <asp:LinkButton type="button" runat="server" ID="BTNAgregaLinea" CommandName="AgregaArticulo" CommandArgument="A4" UseSubmitBehavior="true" CssClass="btn btn-light border border-dark shadow float-end" Style="font-size: 1rem">
                                                            <i class="bi bi-box-arrow-in-right"></i>
                                                                </asp:LinkButton>
                                                            </HeaderTemplate>
                                                            <ItemTemplate>
                    ...
                    <div class="modal-content">
                        ...
                        <div class="modal-body border border-dark border-top-0">
                            <div class="row">
                                <div class="col-lg-2"></div>
                                <div class="col-lg-4">
                                    <label style="width: 100%; text-align: center; font-weight: bold">DISPOSITIVO ANDROID</label>
                                    <asp:Image runat="server" Visible="false" ID="IMGAuxCamara" CssClass="border border-dark rounded rounded-2 mb-2" ImageUrl="..\SMARTH_fonts\INTERNOS\notcamera.png" Width="100%" />
                                    <%--  <button type="button" class="btn btn-outline-dark shadow" style="width: 100%; height: 200px; font-size: 100px" onclick="fully.scanQrCode('Prompt text','javascript:InsertarQR(\'$code\');');"><i class="bi bi-phone"></i></button>--%>
                                    <button type="button" class="btn btn-outline-dark shadow" style="width: 100%; height: 200px; font-size: 100px" onclick="LectoraCamara()"><i class="bi bi-phone"></i></button>

                                </div>
                                <div class="col-lg-4">

                                    <label style="width: 100%; text-align: center; font-weight: bold">LECTORA DE QR</label>
                                    <asp:Image runat="server" Visible="false" ID="Image1" CssClass="border border-dark rounded rounded-2 mb-2" ImageUrl="..\SMARTH_fonts\INTERNOS\notcamera.png" Width="100%" />
                                    <button type="button" class="btn btn-outline-dark shadow" style="width: 100%; height: 200px; font-size: 100px" onclick="LectoraQR()"><i class="bi bi-qr-code-scan"></i></button>
                                </div>
                                <div class="col-lg-2"></div>
                            </div>
                            <div class="row">
                                <h2 runat="server" id="AuxTBUbicacion"></h2>
                                <div class="input-group" id="NUMMaterialManual">
                                    <input class="form-control border border-dark shadow" id="NUMMaterial" runat="server" placeholder="Escribe un producto..." autocomplete="off" style="width:70%">
                                    <input class="form-control border border-dark shadow" id="NUMMaterialLote" type="number" runat="server" placeholder="Escribe el lote" autocomplete="off" style="width:30%">
                                </div>
                                <input class="form-control border border-dark shadow" id="NUMMaterialPistola" runat="server" hidden="true" placeholder="Escanea el código con la pistola de QR." autocomplete="off">
                            </div>
                        </div>
                        <button type="button" id="BTNAgregarMaterial" class="btn btn-success btn-lg border border-dark border-top-0 shadow shadow-sm" style="border-top-left-radius: 0; border-top-right-radius: 0; text-shadow: 1px 1px 2px black" runat="server" onserverclick="AgregarMaterialXUbicacion">Agregar a la ubicación</button>
                    </div>
                </div>
            </div>
        </div>
