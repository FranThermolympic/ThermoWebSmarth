...
{
    public partial class DocumentosGP12 : System.Web.UI.Page
    {
        ...
        public void cargarframes()
        {
            ...
            {
                Conexion_DOCUMENTAL conexion = new Conexion_DOCUMENTAL();
                ds_DocumentosGP12 = conexion.Devuelve_dataset_filtroreferenciasSMARTH(Request.QueryString["REFERENCIA"],"","","","") ;

                if(ds_DocumentosGP12.Tables[0].Rows[0]["PautaControl"].ToString() != "")
                { 
                PautaControl.Attributes.Add("src", ds_DocumentosGP12.Tables[0].Rows[0]["PautaControl"].ToString() + "#zoom=125");
                }
                if (ds_DocumentosGP12.Tables[0].Rows[0]["Defoteca"].ToString() != "")
                {
                    DEFECTOS.Attributes.Add("src", ds_DocumentosGP12.Tables[0].Rows[0]["Defoteca"].ToString() + "#zoom=125");
                }
                if (ds_DocumentosGP12.Tables[0].Rows[0]["Gp12"].ToString() != "")
                {
                    GP12.Attributes.Add("src", ds_DocumentosGP12.Tables[0].Rows[0]["Gp12"].ToString() + "#zoom=125");
                }
                if (ds_DocumentosGP12.Tables[0].Rows[0]["Embalaje"].ToString() != "")
                {
                    PAUTAEMBALAJE.Attributes.Add("src", ds_DocumentosGP12.Tables[0].Rows[0]["Embalaje"].ToString() + "#zoom=125");
                }
            }
            catch (Exception)
            { }
        ...
        protected void CargarNC(object sender, EventArgs e)
        {         
                CargaInicialPorDefecto();
                //CargarDatosNoConformidad("2021001");
                if (Request.QueryString["REFERENCIA"] != null)
                {
                Conexion_SMARTH SHConexion = new Conexion_SMARTH();
                string molde = SHConexion.Devuelve_referenciasXmolde(Request.QueryString["REFERENCIA"].ToString());
                    CargarDatosNoConformidad(molde);
                    if (tbNoConformidad.Text != "")
                    {
                        EXISTEALERTA.Visible = true;
                        EXISTEALERTATABS.Visible = true;
                        NOEXISTEALERTA.Visible = false;
                    }
                }
        }
        ...
        private void CargarDatosNoConformidad(string MOLDE)
        {
            try
            {
                Conexion_SMARTH conexion = new Conexion_SMARTH();
               
                DataSet NC = conexion.Devuelve_datos_NOCONFORMIDAD_MOLDE(MOLDE);
        ...
        public void CargaDatosReferencia(object sender, EventArgs e)
        {
            try
            {
                ...
                {
                    DataSet dtstock = conexion.Devuelve_stock_cuarentena(tbReferenciaCarga.Text);
                    tbCantidadStock.Text = (Convert.ToInt32(dtstock.Tables[0].Rows[0]["CANTALM"]).ToString());
                }
            ...
            try
            {
                ...
                if (!conexion.existe_alerta(tbNoConformidad.Text))
                {
                    if (tbReferenciaCarga.Text != "")
                    {
                        ...
                        //                                Convert.ToInt32(tbReferenciaCarga.Text), conexion.devuelve_ID_Piloto(DropProduccion.SelectedValue), conexion.devuelve_ID_Piloto(DropCalidad.SelectedValue), conexion.devuelve_ID_Piloto(DropIngenieria.SelectedValue), tbFechaOriginal.Text, tbFechaRevision.Text, hyperlink1.ImageUrl, hyperlink2.ImageUrl, hyperlink3.ImageUrl, hyperlink4.ImageUrl, hyperlink5.ImageUrl, tbProcesoAfectado.Text, tbNotas.Text, tbNoConformidadCliente.Text);
                        ...
                        conexion.InsertarNoConformidad(numalerta, tbFechaOriginal.Text, D3.AddDays(1).ToString("dd/MM/yyyy"), D3.AddDays(10).ToString("dd/MM/yyyy"), D3.AddDays(60).ToString("dd/MM/yyyy"), 1, 1, 1, 1);
                        Page.ClientScript.RegisterStartupScript(this.GetType(), "myScript", "creado_OK();", true);
        ...
        public void imprimir_alerta(Object sender, EventArgs e)
        {
            try
            {
                ...
                xlWorkSheet.Cells[5, 5].Value = TipoAlerta.SelectedValue.ToString() + " (" + NivelAlerta.SelectedValue.ToString() + ")"; //de proveedor/de cliente
