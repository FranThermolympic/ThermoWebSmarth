...
{
    public partial class GP12 : System.Web.UI.Page
    {
        ...
        protected void Page_Load(object sender, EventArgs e)
        {
            ...
            else { alertainicio.Style.Value = "display:none"; }
        ...
        public void CargarReferenciaOperario(Object sender, EventArgs e)
        {
            try
            {
                ...
                else
                {
                    ...
                    if (ds.Tables[0].Rows[0]["COSTE"].ToString() != "")
                    {
                ...
                if (ds.Tables[0].Rows[0]["COSTEPIEZA"].ToString() != "")
                {
                ...
                tbResponsable.Text = ds.Tables[0].Rows[0]["Nombre"].ToString();
                ...
                DOCUMENTOS = conexion.devuelve_documentos(tbReferenciaCarga.Text);
                hyperlink4.ImageUrl = DOCUMENTOS.Tables[0].Rows[0]["ImagenPieza"].ToString();
                hyperlink4.NavigateUrl = DOCUMENTOS.Tables[0].Rows[0]["ImagenPieza"].ToString();
        ...
        public void IniciarRevision(Object sender, EventArgs e)
        {
            try
            {
                ...
                alertainicio.Style.Value = "display:block";
        ...
        public void TerminarRevision(Object sender, EventArgs e)
        {
            try
            {
                ...
                if (alertasinfecha.Visible == true)
                    {
                        AlertaGP12 = 1;
                    }
                else if (alertafuera.Visible == true)
                    {
                        AlertaGP12 = 2;
                    }
                else if (alertasinrevision.Visible == true)
                    {
                        AlertaGP12 = 3;
                    }
                ...
                { numlote = TbLote.Text; }
                if (Convert.ToInt32(TotalRevisado.Text) < 1)
                {
                    Page.ClientScript.RegisterStartupScript(this.GetType(), "myScript", "error_cantidad();", true);
                }
                else
                {
                    Conexion_GP12 conexion = new Conexion_GP12();
                    ...
                    conexion.insertarRevision(HoraInicio.Text, HoraFinal.Text, Convert.ToDouble(tbTiempoRevision.Text), conexion.devuelve_IDproveedorrevision(tbEmpresaCarga.Text), Convert.ToInt32(tbOperarioCarga.Text), tbOpeNombreCarga.Text, Convert.ToInt32(tbReferenciaCarga.Text), tbDescripcionCarga.Text, Convert.ToInt32(tbMolde.Text), Convert.ToInt32(numlote), numlote, Convert.ToInt32(TotalRevisado.Text), Convert.ToInt32(TotalOK.Text), Convert.ToInt32(TotalRetrabajado.Text), Convert.ToInt32(TotalNOK.Text),
                                              ...
                                              TbIncidencias.Text, hyperlink1.ImageUrl, hyperlink2.ImageUrl, hyperlink3.ImageUrl, TbObservaciones.Text, OP1, OP2, OP3, OP4, conexion.devuelve_IDlista_razonesrevision(RazonRevision.Text), Convert.ToDouble(CosteRevision.Text), costescrap, costeoperario, AlertaGP12);
                    ...
                    //Comprueba stock en el almacén de CALIDAD para realizar el movimiento
                    try
                    {
                        Conexion_SMARTH SHConexion = new Conexion_SMARTH();
                        if (SHConexion.APP_Almacenes_Activa())
                        {
                            DataTable StockProducto = SHConexion.Devuelve_Stock_ProductoXAlmacen_SERVNAV(tbReferenciaCarga.Text, "CA", "");
                            string ALMPRED = SHConexion.Devuelve_Almacen_PorDefecto(tbReferenciaCarga.Text);

                            //Compara cantidad, Y ejecutar movimientos.
                            int STOCKNAV = Convert.ToInt32(StockProducto.Rows[0]["stock"]);
                            if (STOCKNAV == 0) //SI ES 0 NO HAGO NADA, DEVUELVO ALERTA.
                            {
                                ScriptManager.RegisterStartupScript(this, this.GetType(), "Pop", "AlertaFormato('No hay stock de este producto en el almacén de calidad. No se realizarán movimientos.');", true);
                                mandar_mail_debug("Movimiento GP12 - MovimientosNAV", "Revisión sin stock en almacén");
                            }
                            else if (STOCKNAV < Convert.ToInt32(TotalOK.Text)) // SI EL STOCK ES MENOR, MUEVO EL MÁXIMO POSIBLE DEL ALMACÉN CA, DEVUELVO ALERTA
                            {
                                //PIEZAS OK - Comprueba almacén predeterminado y lo mueve él.                
                                ...
                                string response = SNAV.ChangeLocationItem(tbReferenciaCarga.Text, "CA", ALMPRED, STOCKNAV);
                                ScriptManager.RegisterStartupScript(this, this.GetType(), "Pop", "AlertaFormato('Has declarado más piezas de las existencias del almacén de calidad. Se moverán sólo las disponibles en el almacén.');", true);
                                mandar_mail_debug("Movimiento GP12 - MovimientosNAV", "Cantidad máxima superada");
                            }
                            else // MUEVO LA CANTIDAD BUENA Y DESCARTO LA MALA
                            {
                                //Comprueba almacén predeterminado y lo mueve él.

                                var SNAV = new Serv_NAVISION.WSFunctionsAPP_PortClient();
                                SNAV.ClientCredentials.Windows.ClientCredential = new System.Net.NetworkCredential("Administrador", "eXtint0r");
                                string response = SNAV.ChangeLocationItem(tbReferenciaCarga.Text, "CA", ALMPRED, Convert.ToInt32(TotalOK.Text));
                                mandar_mail_debug("Movimiento GP12 - MovimientosNAV", "Cantidad buena movida");
                                //Muevo las malas
                                try
                                {
                                    if (Convert.ToInt32(TotalNOK.Text) > 0)
                                    {
                                        ...
                                        string CadenaTexto = "" + ";" + tbReferenciaCarga.Text + ";" + numlote.Trim() + ";" + numlote.Trim() + "/00;-" + Convert.ToInt32(TotalNOK.Text).ToString().Replace(",", ".") + ";1;" + DateTime.Now.ToString("dd/MM/yyyy HH:mm:ss") + ";;0";
                                        //string folderPath = @"X:\";
                    ...
                    {
                        mandar_mail_debug("Error en GP12 - MovimientosNAV", ex.Message.ToString());
                    }

                    //Desconectado movimiento 30/10/2023
                    /*
                    try
                    {
                        if (Convert.ToInt32(TotalNOK.Text) > 0)
                ...
                switch (name)
                {
                 ...
                 case "Defecto02":
                   ...
                   case "Defecto08":
                       ...
                       break;
                   case "Defecto09":
                       DefectoDeclarado.Text = "Rayas / Marcas expulsor";
        ...
        */
        ...
        public void mandar_mail(string mensaje, string subject)
        {
            ...
            foreach (DataRow row in ds_correos.Tables[0].Rows)
            {
                ...
                {
                    email.To.Add(new MailAddress(row["Correo"].ToString()));
                }
            }
        ...
        public void mandar_mail_debug(string mensaje, string subject)
        {
            //Create MailMessage
            MailMessage email = new MailMessage();
