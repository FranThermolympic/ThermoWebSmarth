...
{
    public partial class GP12Historicoold : System.Web.UI.Page
    {
        ...
        private void rellenar_grid()
        {
            try
            {
                ...
                /*    double totalBuenas = 0;
                    ...
                    dgv_AreaRechazo.Columns[7].FooterText = totalBuenas.ToString();
                    double totalRetrabajadas = 0;
                    foreach (DataRow dr in ds_Referencias.Tables[0].Rows)
                    {
                        totalRetrabajadas += Convert.ToDouble(dr["Retrabajadas"]);
                    }
                    dgv_AreaRechazo.Columns[8].FooterText = totalRetrabajadas.ToString();
                    double totalMalas = 0;
                    foreach (DataRow dr in ds_Referencias.Tables[0].Rows)
                    {
                        totalMalas += Convert.ToDouble(dr["PiezasNOK"]);
                    }
                    dgv_AreaRechazo.Columns[9].FooterText = totalMalas.ToString();
                    double totalHoras = 0;
                    foreach (DataRow dr in ds_Referencias.Tables[0].Rows)
                    {
                 ...
                 dgv_AreaRechazo.Columns[12].FooterText = totalHoras.ToString();*/
        ...
        protected void cargar_Filtrados(object sender, EventArgs e)
        {
            ...
            ds_Referencias = conexion.devuelve_ultimas_revisiones_filtradas_periodo(selectReferencia.Text, selectMolde.Text, selectLote.Text, cliente, estado, selectFecha.Text, Convert.ToString(TablaSeleccion.SelectedValue), responsable);
            dgv_AreaRechazo.DataSource = ds_Referencias;
            double totalBuenas = 0;
                foreach (DataRow dr in ds_Referencias.Tables[0].Rows)
                {
