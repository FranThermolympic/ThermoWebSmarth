    ...
    public partial class GP12RegistroComunicaciones : System.Web.UI.Page
    {
        ...
        public void rellenar_grid(object sender, EventArgs e)
        {
            try
            {
                ...
                if (selectReferencia.Text != "")
                    {
                    referencia = " AND C.Referencia like '" + selectReferencia.Text.Trim() +"%'";
                    }

                string molde = "";
                if (selectMolde.Text != "")
                    {
                    molde = " AND M.Molde like '" + selectMolde.Text.Trim() + "%'";
                    }

                string lote = "";
                if (selectLote.Text != "")
                    {
                    lote = " AND C.Nlote like '" + selectLote.Text.Trim() + "%'";
                    }
                ...
                ds_Referencias = conexion.Devuelve_Registro_COMUNICACIONES(" AND YEAR([FechaInicio]) = " + Convert.ToString(TablaSeleccion.SelectedValue) + "", referencia,molde,lote, cliente, responsable, operario);
        ...
        public void CargaListasFiltro()
        {
            try
            {
                ...
                DataSet informador = new DataSet();
                informador = conexion.devuelve_setlista_informadores();
                foreach (DataRow row in informador.Tables[0].Rows) { Informador1.Items.Add(row["PInformadores"].ToString()); }
                Informador1.ClearSelection();
                Informador1.SelectedValue = "";

                /*
                foreach (DataRow row in informador.Tables[0].Rows) { Informador2.Items.Add(row["PInformadores"].ToString()); }
                Informador2.ClearSelection();
                Informador2.SelectedValue = "";

                foreach (DataRow row in informador.Tables[0].Rows) { Informador3.Items.Add(row["PInformadores"].ToString()); }
                Informador3.ClearSelection();
                Informador3.SelectedValue = "";

                foreach (DataRow row in informador.Tables[0].Rows) { Informador4.Items.Add(row["PInformadores"].ToString()); }
                Informador4.ClearSelection();
                Informador4.SelectedValue = "";
                */
        ...
        public void GridViewCommandEventHandler(Object sender, GridViewCommandEventArgs e)
        {
            try 
                ...
                {
                    Conexion_GP12 conexion = new Conexion_GP12();
                    DataSet Modal = conexion.modal_detalles_ultimas_revisiones_periodo(e.CommandArgument.ToString(), Convert.ToString(TablaSeleccion.SelectedValue));
                    ...
                    FeedbackOPERARIOS.Text = Modal.Tables[0].Rows[0]["FeedbackOPERARIOS"].ToString();
                    //ACTIVO CELDAS DE OPERARIO Y LAS COMPARO
                    InformadoOp1.Visible = true;
                    Operario1.Visible = true;
                    FechaINFO1.Visible = true;
                    Informador1.Visible = true;
                    InformadoOp1.Visible = true;

                    Operario2.Visible = true;
                    FechaINFO2.Visible = true;
                    Informador2.Visible = true;
                    InformadoOp2.Visible = true;

                    Operario3.Visible = true;
                    FechaINFO3.Visible = true;
                    Informador3.Visible = true;
                    InformadoOp3.Visible = true;

                    Operario4.Visible = true;
                    FechaINFO4.Visible = true;
                    Informador4.Visible = true;
                    InformadoOp4.Visible = true;

                    Operario1.Text = Modal.Tables[0].Rows[0]["OPERARIO1"].ToString();
                    FechaINFO1.Text = Modal.Tables[0].Rows[0]["FechaInfo1"].ToString();
                    Informador1.SelectedValue = Modal.Tables[0].Rows[0]["Informador1"].ToString();
                    if (Operario1.Text == "")
                    {
                        InformadoOp1.Visible = false;
                        Operario1.Visible = false;
                        FechaINFO1.Visible = false;
                        Informador1.Visible = false;
                    }
                    Operario2.Text = Modal.Tables[0].Rows[0]["OPERARIO2"].ToString();
                    FechaINFO2.Text = Modal.Tables[0].Rows[0]["FechaInfo2"].ToString();
                    Informador2.SelectedValue = Modal.Tables[0].Rows[0]["Informador2"].ToString();
                    if (Operario2.Text == "")
                    {
                        InformadoOp2.Visible = false;
                        Operario2.Visible = false;
                        FechaINFO2.Visible = false;
                        Informador2.Visible = false;
                    }
                    Operario3.Text = Modal.Tables[0].Rows[0]["OPERARIO3"].ToString();
                    FechaINFO3.Text = Modal.Tables[0].Rows[0]["FechaInfo3"].ToString();
                    Informador3.SelectedValue = Modal.Tables[0].Rows[0]["Informador3"].ToString();
                    if (Operario3.Text == "")
                    {
                        InformadoOp3.Visible = false;
                        Operario3.Visible = false;
                        FechaINFO3.Visible = false;
                        Informador3.Visible = false;
                    }
                    Operario4.Text = Modal.Tables[0].Rows[0]["OPERARIO4"].ToString();
                    FechaINFO4.Text = Modal.Tables[0].Rows[0]["FechaInfo4"].ToString();
                    Informador4.SelectedValue = Modal.Tables[0].Rows[0]["Informador4"].ToString();
                    if (Operario4.Text == "")
                    {
                        InformadoOp4.Visible = false;
                        Operario4.Visible = false;
                        FechaINFO4.Visible = false;
                        Informador4.Visible = false;
                    }
        ...
        protected void GridView1_DataBound(object sender, EventArgs e)
        {
            ...
            {
                Label lblscrap = (Label)dgv_AreaRechazo.Rows[i].FindControl("lblMalas");
                ...
                Label lblretrabajo = (Label)dgv_AreaRechazo.Rows[i].FindControl("lblRetrabajadas");
        ...
        public void ActualizaInformador (object sender, EventArgs e)
        {
            try
            {
                ...
                {
                    case "InformadoOp1":
                        FechaINFO1.Text = DateTime.Now.ToString("dd/MM/yyy HH:mm");
                        string query = " SET FechaInfo1 = '" + FechaINFO1.Text.ToString() + "', Informador1 = '" + Informador1.SelectedValue.ToString() + "', FeedbackOPERARIOS = '"+FeedbackOPERARIOS.Text.ToString()+"'";
                        conexion.actualizar_GP12_OPERARIOREVISION(Convert.ToInt32(IDINSPECCION.Text), query, Convert.ToString(TablaSeleccion.SelectedValue));
                        break;
                    case "InformadoOp2":
                        FechaINFO2.Text = DateTime.Now.ToString("dd/MM/yyy HH:mm");
                        string query2 = " SET FechaInfo2 = '" + FechaINFO2.Text.ToString() + "', Informador2 = '" + Informador2.SelectedValue.ToString() + "', FeedbackOPERARIOS = '" + FeedbackOPERARIOS.Text.ToString() + "'";
                        conexion.actualizar_GP12_OPERARIOREVISION(Convert.ToInt32(IDINSPECCION.Text), query2, Convert.ToString(TablaSeleccion.SelectedValue));
                        break;
                    case "InformadoOp3":
                        FechaINFO3.Text = DateTime.Now.ToString("dd/MM/yyy HH:mm");
                        string query3 = " SET FechaInfo3 = '" + FechaINFO3.Text.ToString() + "', Informador3 = '" + Informador3.SelectedValue.ToString() + "', FeedbackOPERARIOS = '" + FeedbackOPERARIOS.Text.ToString() + "'";
                        conexion.actualizar_GP12_OPERARIOREVISION(Convert.ToInt32(IDINSPECCION.Text), query3, Convert.ToString(TablaSeleccion.SelectedValue));
                        break;
                    case "InformadoOp4":
                        FechaINFO4.Text = DateTime.Now.ToString("dd/MM/yyy HH:mm");
                        string query4 = " SET FechaInfo4 = '" + FechaINFO4.Text.ToString() + "', Informador4 = '" + Informador4.SelectedValue.ToString() + "', FeedbackOPERARIOS = '" + FeedbackOPERARIOS.Text.ToString() + "'";
                        conexion.actualizar_GP12_OPERARIOREVISION(Convert.ToInt32(IDINSPECCION.Text), query4, Convert.ToString(TablaSeleccion.SelectedValue));
                        break;
                }
        ...
        */
         ...
         protected void cargar_Filtrados(object sender, EventArgs e)
        {
            ...
            //" WHERE C.Referencia like '" + referencia + "%' and M.Molde like '" + molde + "%' and C.Nlote like '" + lote + "%' and CONVERT(VARCHAR(25),FechaInicio,126) like '" + fecha + "%' and R.Razon like '" + razon + "%' and C.NOp1 > 0 ORDER BY FechaInicio DESC", cnn_GP12);


            ds_Referencias = conexion.devuelve_ultimas_revisiones_filtradas_periodo_COMUNICACIONES(selectReferencia.Text, selectMolde.Text, selectLote.Text, cliente, estado, selectFecha.Text, Convert.ToString(TablaSeleccion.SelectedValue), responsable);
            dgv_AreaRechazo.DataSource = ds_Referencias;
